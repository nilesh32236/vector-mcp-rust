...
impl PathGuard {
    ...
    pub fn validate(&self, target: impl AsRef<Path>, op: PathOp) -> Result<PathBuf> {
        ...
        let resolved = match op {
            ...
            PathOp::Create => {
                ...
                let parent = joined.parent()
                    .ok_or_else(|| anyhow::anyhow!("invalid target parent: {}", joined.display()))?;
                ...
                // For simplicity and matching user request "canonicalize the parent directory",
                // we enforce parent exists or its closest existing ancestor.
                ...
                let resolved_parent = std::fs::canonicalize(current)
                    .with_context(|| format!("failed to canonicalize ancestor: {}", current.display()))?;
        ...
        if !resolved.starts_with(&self.base) {
            bail!("path traversal attempt detected: resolved path {} is outside base {}", resolved.display(), self.base.display());
        }
