...
async fn handle_get_code_history(
    ...
    let output = tokio::process::Command::new("git")
        ...
        // SECURITY: Pass PathBuf directly to handle invalid UTF-8, `--` prevents argument injection
        .arg(&abs)
        .current_dir(&root)
        .output()
        .await?;
...
async fn handle_run_linter(server: &Server, params: &CallToolParams) -> Result<CallToolResult> {
    ...
    // SECURITY: Use `--` to separate options from positional arguments (preventing argument injection)
    // and pass PathBuf directly to avoid unsafe UTF-8 conversions.
    let (cmd, args): (&str, Vec<&str>) = match tool.as_str() {
        "go fmt" | "gofmt" => ("gofmt", vec!["-w", "--"]),
        "rustfmt" => ("rustfmt", vec!["--"]),
        "prettier" => ("prettier", vec!["--write", "--"]),
