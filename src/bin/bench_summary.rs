...
fn main() -> Result<()> {
    ...
    if model_path.as_os_str().is_empty() || scenario_path.as_os_str().is_empty() {
        eprintln!(
            "Usage: bench_summary --model <path> --scenarios <path.json> [--output <report.txt>]"
        );
        std::process::exit(1);
    }
    ...
    for scenario in scenarios {
        ...
        if tokens.is_empty() {
            eprintln!(
                "Warning: tokenization produced empty token list for scenario '{}' — skipping",
                scenario.name
            );
            continue;
        }
